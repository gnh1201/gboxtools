...
?>
		...
		<dt>암호화 진행율</dt>
		<dd><span class="badge bg-green">90%</span></dd>
	  </dl>
	</div>
	<!-- /.box-body -->
...
<section>
  <div class="box box-solid">
	<div class="box-header with-border">
	  ...
	  <h3 class="box-title">파일 복호화</h3>
	</div>
	<!-- /.box-header -->
	<form id="form_decryptfile" class="form-horizontal" method="post" action="<?php echo G5_PLUGIN_URL; ?>/boxtools/secure/decrypt_file_update.php">
		<div class="box-body">
			<div>
				<input type="hidden" name="bo_table" value="<?php echo $bo_table; ?>" />
				<input type="hidden" name="wr_id" value="<?php echo $wr_id; ?>" />
				<input type="hidden" name="is_rpc" value="1" />
			</div>
			
			<div class="box-body">
				<div class="form-group">
				  <label for="cert_assign" class="col-sm-2 control-label">복호화키 선택</label>
...
?>
			...
			<div class="box-footer clearfix">
				...
				<button type="submit" class="btn btn-primary pull-right">복호화 시작</a>
			</div>
		</form>
	</div>
</section>
...
$(document).ready(function() {
	...
	// 비동기 복호화
	$("#form_decryptfile").ajaxForm({
		success: showFormResult
	});
...
// 비동기 복호화 이후 과정
function showFormResult(responseText) {
	var res = $.parseJSON(responseText);
	var sub;

	if(res.cnt > 0) {
		if(confirm("복호화가 완료되었습니다.\n\n파일을 다운로드 하시겠습니까?")) {
			sub = $.parseJSON(res.msg);
			
			$.each(sub, function(i, v) {
				var newWin = window.open('', '_blank');
				var decrypted_url = g5_url + "/plugin/boxtools/secure/decrypted_file_down.php?decrypted_id=" + v.dst_num;
				newWin.location = decrypted_url;
